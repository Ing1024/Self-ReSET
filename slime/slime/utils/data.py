    ...
    def __init__(
        ...
        *,
        ...
        for data in read_file(path):
            ...
            pre_rep = data.get("prefill")
            if not isinstance(pre_rep, str):
                pre_rep = None
            if isinstance(pre_rep, str) and pre_rep.lower() == "nan":
                pre_rep = None
            if pd.isna(pre_rep) if 'pd' in globals() else (pre_rep != pre_rep):  # check nan
                pre_rep = None
            # print(f"data: {data}",flush=True)
            # print(f"pre_rep: {pre_rep}",flush=True)
            pre_rep_length = len(pre_rep) if pre_rep is not None else 0
            if pre_rep is None:
                ...
                self.origin_samples.append(
                    Sample(
                        prompt=prompt,
                        label=data[label_key] if label_key is not None else None,
                        response=pre_rep,# for RECAP
                        pre_rep_length=pre_rep_length,
                        metadata=data.get(metadata_key) or {},
                    )
                )
        self.epoch_id = -1
