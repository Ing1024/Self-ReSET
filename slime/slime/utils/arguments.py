    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            parser.add_argument(
                ...
                type=float,
                default=0.0,
                help="Whether to flip the label of the response with the probability of label-flip.",
            )
            parser.add_argument(
                "--use-llm-judge",
                action="store_true",
                help="Whether to use LLM-as-judge for over-refusal detection.",
            )
            parser.add_argument(
                "--enable-partial-mask",
                action="store_true",
                help="Whether to enable recap.",
            )
            parser.add_argument(
                "--enable-noresp",
                action="store_true",
                help="Whether to enable no response.",
            )
            parser.add_argument(
                "--no-save",
                action="store_true",
                help="Whether to delete the old checkpoints.",
            )
            parser.add_argument(
                "--buffer-max-length",
                type=int,
                default=200,
                help="The maximum length of the buffer.",
            )
            parser.add_argument(
                "--refusal-in-buffer-threshold",
                type=int,
                default=0,
                help="The threshold of the refusal in buffer.",
            )
            parser.add_argument(
                "--use-llama-guard",
                action="store_true",
                help="Whether to use llama guard for evaluation.",
            )
            parser.add_argument(
                "--enable-unsafe2-truncate",
                action="store_true",
                help="Whether to enable truncate the response after the first two unsafe/controversial tokens.",
            )
            parser.add_argument(
                "--enable-add-after",
                action="store_true",
                help="Whether to enable add after the first unsafe/controversial token.",
            )
            parser.add_argument(
                "--enable-punctuation-after",
                action="store_true",
                help="Whether to enable punctuation after the first unsafe/controversial token.",
            )
            parser.add_argument(
                "--enable-dynamic-filter",
                action="store_true",
                help="Whether to enable dynamic filter.",
            )
            parser.add_argument(
                "--llama-guard-model-path",
                type=str,
                default=None,
                help="The path to the llama guard model to check the process safety.",
            )
            parser.add_argument(
                "--eval-model-path",
                type=str,
                default=None,
                help="The path to the eval model to check the process safety.",
            )
            parser.add_argument(
                "--refusal-in-buffer",
                    action="store_true",
                    help="Whether to put the refusal sample in buffer",
            )
            parser.add_argument(
                "--enable-partialrollout",
                action="store_true",
                help="Whether to enable partial rollout.",
            )
            parser.add_argument(
                "--stream-unsafe-final-safe",
                action="store_true",
                help="Whether to stream unsafe final safe.",
            )
            parser.add_argument(
                "--enable-safety-label-thinking",
                action="store_true",
                help="Whether to enable checking the label reasoning process",
            )   
            parser.add_argument(
                "--enable-safety-thinking",
                action="store_true",
                help="Whether to enable checking the reasoning process",
            )
            parser.add_argument(
                "--safety-model-num-gpus",
                type=int,
                default=1,
                help="The number of GPUs for the safety stream model.",
            )
            parser.add_argument(
                "--safety-guard-model-path",
                type=str,
                default=None,
                help="the path to the safety guard model to check the process safety.",
            )
            parser.add_argument(
                "--safety-stream-model-path",
                type=str,
                default=None,
                help="The path to the safety stream model to check the process safety.",
            )
            parser.add_argument(
                "--guard-service-url",
                type=str,
                default=None,
                help="URL of external guard model HTTP service (e.g. http://localhost:8100). "
                     "When set, guard models are accessed via HTTP and RolloutManager needs 0 GPUs.",
            )
        ...
        def add_algo_arguments(parser):
            ...
            reset_arg(parser,"--save-list", type=list, default=None)
...
def _resolve_eval_datasets(args) -> list[EvalDatasetConfig]:
    ...
    if args.eval_config:
        ...
        for dataset in datasets_config:
            print(f"Dataset: {dataset['name']}, Path: {dataset['path']}", flush=True)
    else:
        datasets_config = []
