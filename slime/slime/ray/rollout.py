...
class RolloutManager:
    ...
    def __init__(self, args, pg, wandb_run_id):
        ...
        # custom: create the content evaluator
        if getattr(args, 'guard_service_url', None):
            # External guard service mode - no local GPU needed
            from slime.rollout.GuardforwardRemote import RemoteContentEvaluator, RemoteGenLabelEvaluator
            self.content_evaluator = RemoteContentEvaluator(args.guard_service_url) if args.safety_stream_model_path else None
            self.gen_label_evaluator = RemoteGenLabelEvaluator(args.guard_service_url) if args.safety_guard_model_path else None
            print(f"[RolloutManager] Using external guard service at {args.guard_service_url}")
        else:
            # Local mode - load models on this GPU
            if args.safety_stream_model_path is not None:
                self.content_evaluator = ContentEvaluator(args.safety_stream_model_path)
            else:
                self.content_evaluator = None
            if args.safety_guard_model_path is not None:
                self.gen_label_evaluator = GenLabelEvaluator(args.safety_guard_model_path, args.eval_model_path if args.eval_model_path is not None else args.safety_guard_model_path, args.llama_guard_model_path)
            else:
                self.gen_label_evaluator = None
    ...
    def get_epoch_now(self):
        return self.data_source.epoch_now

    def get_epoch_id(self):
        return self.data_source.epoch_id
    ...
    def eval(self, rollout_id):
        ...
        data = call_rollout_fn(
            self.eval_generate_rollout, self.args, rollout_id, self.data_source, self.content_evaluator,self.gen_label_evaluator, evaluation=True
        ).data
    ...
    def _get_rollout_data(self, rollout_id):
        if self.args.load_debug_rollout_data:
            ...
            data = call_rollout_fn(self.generate_rollout, self.args, rollout_id, self.data_source, self.content_evaluator, self.gen_label_evaluator, evaluation=False)
    ...
    def _convert_samples_to_train_data(self, samples: Union[list[Sample], list[list[Sample]]]):
        ...
        for sample in samples:
            ...
            # for RECAP, apply pre_rep_length mask if present
            # Check if pre_rep_length exists as attribute or in metadata
            pre_rep_length = getattr(sample, 'pre_rep_length', None)
            if pre_rep_length is None and sample.metadata:
                pre_rep_length = sample.metadata.get('pre_rep_length', None)
            
            # If pre_rep_length is not zero, mask the first pre_rep_length tokens
            if pre_rep_length is not None and pre_rep_length > 0:
                # Ensure pre_rep_length doesn't exceed response_length
                mask_length = min(pre_rep_length, sample.response_length)
                # Set first mask_length tokens to 0, keep the rest as is
...
def _compute_zero_std_metrics(args, all_samples: List[Sample]):
    ...
    partial_sample_groups = [g for g in all_sample_groups.values() if g[0].partialrollout] # +
    
    partial_interesting_sample_groups = [g for g in interesting_sample_groups if g[0].partialrollout] # +
    
    partial_rewards = [str(round(g[0].get_reward_value(args), 1)) for g in partial_interesting_sample_groups] # +

    interesting_rewards = [str(round(g[0].get_reward_value(args), 1)) for g in interesting_sample_groups]


    zero_std_metrics = {f"zero_std/count_{reward}": len(items) for reward, items in group_by(interesting_rewards).items()}
    
    partial_nums_metrics = {f"partial_group/count": len(partial_sample_groups)} # +
    partial_std_metrics = {f"partial_zero_std/count_{reward}": len(items) for reward, items in group_by(partial_rewards).items()} # +
     
    return {**zero_std_metrics, **partial_nums_metrics, **partial_std_metrics}
